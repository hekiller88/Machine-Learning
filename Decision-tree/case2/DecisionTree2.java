...
// Java Programmer Employment
// attribute: CS degree or not;
//			  Java Knowledge
//			  C/C++ Knowlege
//			  gpa > 3 ?
...
public class DecisionTree2 {
	...
	private String entropyRule; // option: gini, class, shannon
	...
	/*
	 * @output: [0] majority class name [1] majority class amount
	 */
	...
	// option = 1 - gini entropy
	// 2 - class error entropy
	// x - Shannon's entropy
	private double entropy(ArrayList<Record> records) {
										...
										// ...)
		{
			...
			return 1 - sum;
		} else if (entropyRule.equals("Class")) // class(s) = 1 - max(p1, p2,
												// p3, ...)
		{
			double[] frequency = new double[numberClasses]; // frequency array
	...
	public void classifyData(String testFile, String classifiedFile) throws IOException {
		...
		for (int i = 0; i < numberRecords; i++) {
			...
			outFile.println("Class: " + convert(found.className) + "\tConfidence Fraction of Records in Leaf:  "
					+ String.format("%.2f", found.confidenceRec) + "\n\t\tConfidence Fraction of Majority in Leaf: "
					+ String.format("%.2f", found.confidenceMaj));

			System.out.println("Class: " + convert(found.className) + "\tConfidence Fraction of Records in Leaf:  "
					+ String.format("%.2f", found.confidenceRec) + "\n\t\tConfidence Fraction of Majority in Leaf: "
					+ String.format("%.2f", found.confidenceMaj));
		}
	...
	public double leaveOneOutValidation(String trainingFile) throws IOException {
		...
		for (int i = 0; i < numberRecords; i++) {
			...
			DecisionTree2 cutOutTree = new DecisionTree2();
